  ...
  const handleSubmit = async (sendConfirmation: boolean) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          res.status >= 500
            ? "Server error while punching lead. Please try again in a moment."
      ...
      if (!res.ok) {
        ...
        if (res.status === 409 && data.code === "PRIOR_CP_COOLDOWN") {
          addToast({
            ...
            message:
              (typeof data.error === "string" && data.error)
              || "You previously owned this lead during a lock. Wait for the 7-day cooldown before punching again.",
          });
          return;
        }
        throw new Error((data.error as string) || "Failed to punch lead");
      }
