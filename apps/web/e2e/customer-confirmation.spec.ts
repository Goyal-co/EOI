...
test.describe("Customer Confirmation Flow", () => {
  test("confirm accept page loads for demo token", async ({ page }) => {
    ...
    await expect(page.getByText(/Goyal|Confirm|Accept/i).first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("confirm reject page loads", async ({ page }) => {
    ...
    await expect(page.getByText(/Goyal|Reject|Decline/i).first()).toBeVisible({ timeout: 10_000 });
  });
});
